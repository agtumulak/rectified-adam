...
def createModel(args, r):
    ...
    model.compile(loss='categorical_crossentropy', optimizer=SGD(lr=r), metrics=['accuracy'])
...
def train(args):
    ...
    for color, rate in learning.items():
        i += 1
        modelPath = trainPath + "bestModelSGD{}.hdf5".format(rate)
    ...
    plt.savefig(trainPath + "SGD_accuracy_LR.jpg")
    ...
    plt.title("SGD Accuracy Metrics for Varying Learning Rates")
    ...
    plt.savefig(trainPath + "SGD_Bar.jpg")
    plt.show()
    ###########################################################
...
def getAccuracy(preds, testF, testC):
    ...
    print(" ----- SGD accuracy on Fine Labels: {}%".format(fineAcc))
    print(" ----- SGD Top 3 accuracy on Fine Labels: {}%".format(top3Acc))
    print(" ----- SGD accuracy on Coarse Labels: {}%".format(coarseAcc))
    print('\n')
