...
def createModel(args, r):
    ...
    model.compile(loss='categorical_crossentropy', optimizer=Adam(lr=r), metrics=['accuracy'])
...
def train(args):
    ...
    for color, rate in learning.items():
        i += 1
        modelPath = trainPath + "bestModelAdam{}.hdf5".format(rate)
    ...
    plt.savefig(trainPath + "Adam_accuracy_LR.jpg")
    ...
    plt.ylabel("Accuracy")
    plt.legend(("Fine Labels", "Fine Labels (Top 3)", "Coarse Labels"), loc="best")
    plt.title("Adam Accuracy Metrics for Varying Learning Rates")
    ...
    plt.savefig(trainPath + "Adam_Bar.jpg")
    plt.show()
    ###########################################################
...
def getAccuracy(preds, testF, testC):
    ...
    print(" ----- Adam accuracy on Fine Labels: {}%".format(fineAcc))
    print(" ----- Adam Top 3 accuracy on Fine Labels: {}%".format(top3Acc))
    print(" ----- Adam accuracy on Coarse Labels: {}%".format(coarseAcc))
    print('\n')
